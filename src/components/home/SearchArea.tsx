...
import { useNavigate } from 'react-router';   /* useSearchParams */

export default function SearchArea() {
  const navigate = useNavigate();
  // const [searchParams] = useSearchParams();   주석 처리함
  // const [query, setQuezry] = useState(() => searchParams.get('q') || '');
  ...
  useEffect(() => {
    ...
    debounceTimer.current = setTimeout(() => {  // 디바운스: 입력 300ms 후에 검색 수행
      navigate(query ? '?q=' + query : '/');
    }, 300);
    return () => {  // 클린업: 컴포넌트 언마운트 시 타이머 취소
      if (debounceTimer.current) {
        ...
        <article className='search-area__search'>
          ...
          >
            <input
              ...
              className='search-area__input' autoComplete='off'
