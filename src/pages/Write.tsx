    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        const { status } = await axiosInstance.post('/posts', {
          title, category, thumbnail, desc, username,
        });
        if (status === 201) {
          alert('글이 등록되었습니다.');
          navigate('/');
            ...
            <div className='page__write-form'>
              <div className='page__write-group'>
                ...
                <input type='text' name='title' id='title'
                  className='page__write-input' placeholder='Type product name' 
                  value={title} onChange={(e) => setTitle(e.target.value)}
                  required />
              </div>
              <div>
                <label htmlFor='category' className='page__write-label'>카테고리</label>
                <select id='category' className='page__write-select' 
              ...
              <div className='page__write-group'>
                <div>
                  ...
                  <input className='page__write-file' aria-describedby='user_avatar_help'
                    id='user_avatar' type='file' onChange={handleFileChange} 
                    accept='image/*' required />
                </div>
              </div>
              <div className='page__write-group'>
                <label htmlFor='description' className='page__write-label'>내용</label>
                <textarea id='description' className='page__write-textarea'
